  ...
  run: async ({ msg, sock }) => {
    ...
    if (!userId) {
      return sock.sendMessage(msg.chat, {
        text: "❌ No se pudo detectar el usuario."
      }, { quoted: msg });
    }
    ...
    // 🔥 FIX ULTRA: captura TODOS los formatos posibles de logros
    const listaLogros = (user.achievements || []).map((a, i) => {

      const name = a?.name || a?.title || "Logro desconocido";
      const emoji = a?.emoji || "🏅";
