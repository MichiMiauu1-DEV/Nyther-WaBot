...
// 🔧 NORMALIZADOR (NO TOCA PAISES, SOLO INPUT)
...
// 🌍 TODAS TUS BANDERAS (NO SE QUITÓ NADA)
const flags = [
  ...
  run: async ({ msg, sock }) => {
    ...
    if (g.flagActive) {
      ...
      if (game.blocked.includes(userId)) {
        return sock.sendMessage(chat, { text: "❌ Ya perdiste esta partida." }, { quoted: msg });
      }
...
🎯 Victorias: ${u.flagWins}`;
      ...
      if (game.attempts[userId] <= 0) {
        ...
        return sock.sendMessage(chat, {
          text: `❌ Perdiste. La respuesta era: ${game.answer}`
        }, { quoted: msg });
      }
